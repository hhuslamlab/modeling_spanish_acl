...
def halign(s: str, t: str) -> Tuple[str, str]:
...
def levenshtein(s: str, t: str, inscost: float = 1.0, delcost: float = 1.0, substcost: float = 1.0) -> Tuple[str, str, float]:
    """Recursive implementation of Levenshtein, with alignments returned."""
    @memolrec
    def lrec(spast: str, tpast: str, srem: str, trem: str, cost: float) -> Tuple[str, str, str, str, float]:
        if len(srem) == 0:
...
def memolrec(func: Callable) -> Callable:
    """Memoizer for Levenshtein."""
    cache = {}
    @wraps(func)
    def wrap(sp: str, tp: str, sr: str, tr: str, cost: float) -> Tuple[str, str, str, str, float]:
...
def alignprs(lemma: str, form: str) -> Tuple[str, str, str, str, str, str]:
...
def prefix_suffix_rules_get(lemma: str, form: str) -> Tuple[set, set]:
...
def apply_best_rule(lemma: str, msd: str, allprules: set, allsrules: set) -> str:
...
def numleadingsyms(s: str, symbol: str) -> int:
    return len(s) - len(s.lstrip(symbol))


def numtrailingsyms(s: str, symbol: str) -> int:
    return len(s) - len(s.rstrip(symbol))

def main(argv: List[str]) -> None:
    options, remainder = getopt.gnu_getopt(argv[1:], 'ohp:l:', ['output','help','path=','lang='])
    ...
    if HELP or lang is None:
        print("\n*** Single Language Morphological Analyzer ***\n")
        print("Options:")
        print(" -o create output files with guesses")
        print(" -p [path] data files path. Default is ../data/")
        print(" -l [lang] language to process (required)")
        quit()

    allprules, allsrules = {}, {}

    if not os.path.isfile(path + lang + ".trn"):
        print(f"Error: Training file for {lang} not found")
        return

    lines = [line.strip() for line in open(path + lang + ".trn", "r", encoding='utf8') if line != '\n']

    # Check if language is predominantly suffixing or prefixing
    prefbias, suffbias = 0, 0
    for l in lines:
        lemma, _, form = l.split('\t')
    ...
    for l in devlines:
        ...
        if OUTPUT:
            outfile.write(f"{lemma}\t{msd}\t{outform}\n")

    if OUTPUT:
        outfile.close()

    print(f"{lang} accuracy: {numcorrect/float(numguesses):.6f}")
