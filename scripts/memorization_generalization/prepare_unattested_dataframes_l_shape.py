"""
prepare dataset for logistic regression for L shape verbs for unattested
"""
...
def get_train_triples_l(
    train_tgts: List[str], train_triples: List[List[str]]
) -> List[str]:
    train_triples_l = []
    for tgt, train_triple in zip(train_tgts, train_triples):
        ...
        if shape == "L":
            train_triples_l.append(train_triple)

    return train_triples_l
...
if __name__ == "__main__":
    ...
    for model in input_condition:
        ...
        total_predictions_l = []
        total_predictions_unattested_l = []
        total_prediction_status_unattested_l = []
        total_test_unattested_l = []
        total_prediction_status_attested_l = []
        total_test_l = []
        condition = []
        run = []
        models = []
        ...
        train_triples_l = get_train_triples_l(train_tgts, train_triples)
        print("num. of L-shaped train triples: ", len(train_triples_l))
        ...
        for (
            ...
            if shape == "L" and test_from_tgt_sf not in exception_forms:
                total_predictions_l.append(pred)
                total_test_l.append(test_from_tgt_sf)
                ...
                if test_triple not in train_triples_l:
                    condition.append(model.split("_")[0] + "_" + model.split("_")[1])
                    run.append(model.split("_")[2])
                    models.append(model)
                    total_unattested_l.append(test_triple)
                    total_predictions_unattested_l.append(pred)
                    total_test_unattested_l.append(test_from_tgt_sf)
                    if pred == test_from_tgt_sf:
                        total_prediction_status_unattested_l.append(1)
                        seen_triples_correct_predictions.append(test_triple)
                    if pred != test_from_tgt_sf:
                        seen_triples_wrong_predictions.append(test_triple)
                        total_prediction_status_unattested_l.append(0)
        ...
        freq_train_triples = dict(Counter(total_unattested_l))

        for item in total_unattested_l:
            freq_info.append(int(freq_train_triples[item]))

        print(
            "total predictions l: ",
            len(total_predictions_l),
            "len of prediction status: ",
            len(total_prediction_status_unattested_l),
            "len of frequency info: ",
            len(freq_info),
            "len of triples: ",
            len(total_attested_l),
            "len of prediction: ",
            len(total_predictions_unattested_l),
            "len of test: ",
            len(total_test_unattested_l),
        )
        if (
            not total_prediction_status_unattested_l
            and not total_unattested_l
            and not total_predictions_unattested_l
            and not total_test_unattested_l
        ):
            total_prediction_status_unattested_l.append(0)
            total_attested_l.append(0)
            freq_info.append(0)
            total_unattested_l.append(0)
            total_test_unattested_l.append(0)
            total_predictions_unattested_l.append(0)
            condition.append(model.split("_")[0] + "_" + model.split("_")[1])
            run.append(model.split("_")[2])
            models.append(model)
        ...
        df["run"] = run
        df["model"] = models
        df["prediction_status"] = total_prediction_status_unattested_l
        df["frequency"] = freq_info
        df["triples"] = total_unattested_l
        df["prediction"] = total_predictions_unattested_l
        df["test"] = total_test_unattested_l
