...
def get_train_triples(model) -> List[List[str]]:
...
def get_train_triples_l(
    train_tgts: List[str], train_triples: List[List[str]], lshaped_forms: List[str]
) -> List[str]:
    train_triples_l = []
...
if __name__ == "__main__":
    ...
    for model in input_condition:
        ...
        new_df.to_csv(
            "../../data/fixed_run/analysis/memorization_generalization/l_shape/section_6_4_1/attested/triples_"
            + train_triples_set
            + "/"
            + condition
            + "/"
