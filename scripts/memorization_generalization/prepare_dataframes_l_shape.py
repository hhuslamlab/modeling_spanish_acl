...
if __name__ == "__main__":
    ...
    for model in input_condition:
        ...
        total_predictions_l = []
        total_predictions_attested_l = []
        total_test_attested_l = []
        ...
        for (
            ...
            if shape == "L":
                ...
                if test_triple in train_triples_l:
                    ...
                    total_predictions_attested_l.append(pred)
                    total_test_attested_l.append(test_from_tgt_sf)
                    if pred == test_from_tgt_sf:
                        total_prediction_status_attested_l.append(1)
                        seen_triples_correct_predictions.append(test_triple)
                    if pred != test_from_tgt_sf:
                        seen_triples_wrong_predictions.append(test_triple)
                        total_prediction_status_attested_l.append(0)
        ...
        freq_train_triples = dict(Counter(train_triples_l))
        for item in total_attested_l:
            freq_info.append(int(freq_train_triples[item]))
        print(
            "total predictions l: ",
            len(total_predictions_l),
            "len of prediction status: ",
            len(total_prediction_status_attested_l),
            "len of frequency info: ",
            len(freq_info),
            "len of triples: ",
            len(total_attested_l),
            "len of prediction: ",
            len(total_predictions_attested_l),
            "len of test: ",
            len(total_test_attested_l),
        )
        df["condition"] = condition
        df["run"] = run
        df["model"] = models
        df["prediction_status"] = total_prediction_status_attested_l
        df["frequency"] = freq_info
        df["triples"] = total_attested_l
        df["prediction"] = total_predictions_attested_l
        df["test"] = total_test_attested_l
