...
if __name__ == "__main__":
    ...
    for model in input_condition:
        ...
        total_predictions_attested_nl = []
        total_test_attested_nl = []
        ...
        train_triples_nl = get_train_triples_nl(train_tgts, train_triples)
        ...
        for (
            ...
            if shape == "NL":
                ...
                if test_triple in train_triples_nl:
                    condition.append(model.split("_")[0] + "_" + model.split("_")[1])
                    run.append(model.split("_")[2])
                    models.append(model)
                    total_attested_nl.append(test_triple)
                    total_predictions_attested_nl.append(pred)
                    total_test_attested_nl.append(test_from_tgt_sf)
                    if pred == test_from_tgt_sf:
                        total_prediction_status_attested_nl.append(1)
                        seen_triples_correct_predictions.append(test_triple)
                    if pred != test_from_tgt_sf:
                        seen_triples_wrong_predictions.append(test_triple)
                        total_prediction_status_attested_nl.append(0)
        ...
        freq_train_triples = dict(Counter(train_triples_nl))
        for item in total_attested_nl:
            freq_info.append(int(freq_train_triples[item]))
        print(
            "total predictions nl: ",
            len(total_predictions_nl),
            "len of prediction status: ",
            len(total_prediction_status_attested_nl),
            "len of frequency info: ",
            len(freq_info),
            "len of triples: ",
            len(total_attested_nl),
            "len of prediction: ",
            len(total_predictions_attested_nl),
            "len of test: ",
            len(total_test_attested_nl),
        )
        df["condition"] = condition
        df["run"] = run
        df["model"] = models
        df["prediction_status"] = total_prediction_status_attested_nl
        df["frequency"] = freq_info
        df["triples"] = total_attested_nl
        df["prediction"] = total_predictions_attested_nl
        df["test"] = total_test_attested_nl
